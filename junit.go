...
func (ts *TestSuite) Compute() {
	...
	for i := range ts.TestCases {
		...
		case tc.IsFailure():
			ts.Failures++
		case tc.IsSkipped(): // we favor skipped over disabled, so ignore disabled
			ts.Skipped++
		}
	}
}
...
func (ts *TestSuites) TestCases() []TestCase {
	var tcs []TestCase
	for i := range ts.TestSuites {
		tcs = append(tcs, ts.TestSuites[i].TestCases...)
	}
...
func Parse(data []byte) (TestSuites, error) {
	...
	if err != nil {
		...
		var ts TestSuite
		if serr := xml.Unmarshal(data, &ts); serr != nil {
			return tss, err
		}
...
// MergeTestSuites merges multiple test reports into a single report.
func MergeTestSuites(reports ...TestSuites) TestSuites {
	suites := make(map[string]TestSuite)

	for r := range reports {
		report := reports[r]
		for s := range report.TestSuites {
			suite := report.TestSuites[s]
